...
META_FILES := \
	$(BRANCH_FILE) \
	$(SHA_FILE) \
	$(TOP)/tmp/docker-meta/.dumped \
	$(TOP)/tmp/job-board-env/.dumped
...
CHEF_COOKBOOK_PATH := $(TOP)/.git::cookbooks \
...
%: %.yml $(META_FILES)
	$(PACKER) build -only=$(BUILDER) <($(TOP)/bin/yml2json < $<)
...
.PHONY: stacks-trusty
stacks-trusty:
	@$(TOP)/bin/list-stacks trusty

.PHONY: stacks-xenial
stacks-xenial:
	@$(TOP)/bin/list-stacks xenial
...
update-gce-images:
	$(TOP)/bin/gce-image-update $$(git grep -lE 'source_image: ubuntu' *.yml)

tmp/packer.zip:
	$(CURL) -sSLo $@ 'https://releases.hashicorp.com/packer/1.2.4/packer_1.2.4_$(UNAME)_amd64.zip'
...
$(META_FILES): .git/HEAD
	$(TOP)/bin/dump-git-meta ./tmp/git-meta
	$(TOP)/bin/write-envdir $(TOP)/tmp/docker-meta 'DOCKER_LOGIN_(USERNAME|PASSWORD|SERVER)'
	$(TOP)/bin/write-envdir $(TOP)/tmp/job-board-env 'JOB_BOARD'

packer-assets/%-system-info-commands.yml: $(wildcard packer-assets/system-info.d/*.yml)
	chmod 0600 $@
	$(TOP)/bin/generate-system-info-commands \
